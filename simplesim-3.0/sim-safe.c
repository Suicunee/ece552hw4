/* Xi Chen      xc98@duke.edu
 * Hang Yuan    hy117@duke.edu
 * Yuchen Qian  yq45@duke.edu   
 */
...
/* ECE552 track number of loads */
static counter_t sim_num_load = 0;

/* ECE552 track number of FP isntructions */
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_refs",
		   ...
		   &sim_num_refs, 0, NULL);
  /* ECE552 set up counter for FP load */
  stat_reg_counter(sdb, "sim_num_load",
		   "total number of FP loads executed",
		   &sim_num_load, 0, NULL);
  /* ECE552 set up counter for FP instructions */
  stat_reg_counter(sdb, "sim_num_fp_insn",
		   "total number of FP instructions executed",
		   &sim_num_fp_insn, 0, NULL);
...
sim_init(void)
{
  ...
  /* ECE552 initialize simulator counter for FP load and instructions*/
  sim_num_fp_insn = 0;
  sim_num_load = 0;
...
#include "machine.def"
	...
	/* ECE552 check the op code and count number of FP instructions */
	 if (MD_OP_FLAGS(op) & F_FCOMP) {
	 	sim_num_fp_insn++;
	 }

   /* ECE552 check the op code and count number of loads */
